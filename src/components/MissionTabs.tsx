...
export default function MissionTabs() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-[1100]">
      <div className="mx-auto w-full max-w-md sm:max-w-lg md:max-w-xl lg:max-w-2xl px-3 pb-[max(env(safe-area-inset-bottom),20px)]">
        <div className="h-20 rounded-3xl border border-white/20 bg-white/80 shadow-lg backdrop-blur supports-[backdrop-filter]:bg-white/60">
          <div className="grid h-full grid-cols-4 items-center px-1.5">
            {tabs.map(({ to, label, Icon }) => (
              <NavLink
                key={to}
                to={to}
                className={({ isActive }: { isActive: boolean }) =>
                  `relative mx-1 flex h-16 flex-col items-center justify-center gap-1 rounded-3xl px-2.5 transition-colors ${
