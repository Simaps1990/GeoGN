...
export default function MissionTabs() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-[1100]">
      <div className="mx-auto w-full max-w-md sm:max-w-lg md:max-w-xl px-3 pb-[max(env(safe-area-inset-bottom),16px)]">
        <div className="h-16 rounded-[25px] border border-white/20 bg-white/80 shadow-lg backdrop-blur supports-[backdrop-filter]:bg-white/60">
            ...
            {tabs.map(({ to, label, Icon }) => (
              <NavLink
                ...
                className={({ isActive }: { isActive: boolean }) =>
                  `relative mx-1 flex h-12 flex-col items-center justify-center gap-1 rounded-[18px] px-2 transition-colors ${
